...
document.querySelector(".perc").addEventListener("click", (e) => {
  if (number && !number2 && !action) {
    ...
    number = num.toString();
    calcArea.value =
      calcArea.value.substring(0, calcArea.value.length - +prevStr) + number;
    transformFont();
  } else if (number && number2) {
    let { num, prevStr } = converToPerc(number2);
    number2 = num.toString();
    calcArea.value =
      calcArea.value.substring(0, calcArea.value.length - +prevStr) + number2;
    transformFont();
  }
});
...
function validateActions(currentAction) {
  ...
  let replaceAcBfNum2 =
    number != "" && action && !action.includes(currentAction) && number2 === "";
  let isRepeatedBfNum2 =
    number != "" && action.includes(currentAction) && number2 === "";
  ...
  if (replaceAcBfNum1) {
    ...
    action = currentAction;
    calcArea.value =
      calcArea.value.substring(0, calcArea.value.length - 1) + action;
  } else {
    if (isCalc) {
      calcSum();
      number = sum.toString();
...
function validateNumbers(num, currentValue) {
  ...
  if (isDot) {
    ...
    calcArea.value =
      calcArea.value.substring(0, calcArea.value.length - 1) + currentValue;
    return currentValue;
  } else {
    if (!number && action) {
      number = "0";
...
function checkResult(res) {
  if (res === "You can't divide by zero") {
    showError(res);
  } else if (isNaN(res)) {
